...
export const MetricCard = ({ label, projects, total, color, onClick }: MetricCardProps) => {
    ...
    return (
        <motion.div
            ...
            className={`p-5 rounded-2xl bg-white/80 dark:bg-[#1e1e1e]/80 backdrop-blur-sm border border-gray-200 dark:border-white/5 flex flex-col justify-between ${onClick ? 'hover:border-gray-300 dark:hover:border-white/20 cursor-pointer hover:scale-[1.02] active:scale-[0.98]' : ''} transition-all duration-200 shadow-sm dark:shadow-none h-full min-h-[140px]`}
            onClick={() => onClick && onClick(label, projects)}
        >
            {/* Top section: Label and main value */}
            <div>
                <h3 className="text-gray-500 dark:text-gray-400 mb-3 font-medium text-sm">{label}</h3>
