...
const DashboardContent = () => {
  const { projects, logs, activeYears, availableYears, setActiveYears } = useData();
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (activeYears && activeYears.length > 0) {
      filtered = filtered.filter(p => activeYears.includes(p.reportingYear));
    }
    if (activeSector !== "All Sectors") {
      filtered = filtered.filter(p => p.sector === activeSector);
    }
    return filtered;
  }, [projects, activeSector, activeYears]);
  ...
  return (
    <DashboardLayout>
      {/* Sector and Year Navigation */}
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-8">

        {/* Sector Tabs */}
        <div className="flex gap-2 w-full overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/10">
          {TABS.map(tab => (
            <button
              key={tab.id}
            ...
            {availableYears.map(year => {
              ...
              return (
                <button
                  key={year}
                  onClick={() => {
                    if (isActive && activeYears.length > 1) {
                      setActiveYears(activeYears.filter(y => y !== year));
                    } else if (!isActive) {
                      setActiveYears([...activeYears, year].sort((a, b) => b - a));
                    }
                  }}
                  className={`px-4 py-1.5 text-sm font-bold rounded-lg transition-all duration-200 ${isActive ? 'bg-indigo-600 text-white shadow-md scale-105' : 'text-gray-600 dark:text-gray-400 hover:bg-white dark:hover:bg-white/10 hover:text-gray-900 dark:hover:text-white'}`}
                >
                  {year}
                </button>
              );
      ...
      >
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-2">
              ...
              <motion.div
                variants={{ hidden: { y: 20, opacity: 0 }, visible: { y: 0, opacity: 1 } }}
                className="p-6 rounded-2xl bg-white/80 dark:bg-[#1e1e1e]/80 backdrop-blur-sm border border-gray-200 dark:border-white/5 flex flex-col justify-center hover:border-gray-300 dark:hover:border-white/20 transition-all duration-200 shadow-sm dark:shadow-none cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
                onClick={() => setSelectedMetric({ title: "Energy Modeled Projects", projects: filteredProjects })}
              >
                ...
                <div className="flex items-center justify-between mt-1 h-full">
                  <div className="flex flex-col">
                    ...
                    <p className="text-3xl font-bold text-green-600 dark:text-green-400">
                      {filteredProjects.filter(p => p.energyModel === 'Yes').length}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="text-3xl font-bold text-red-500 dark:text-red-400">
                      {filteredProjects.filter(p => p.energyModel === 'No').length}
                    </p>
                  </div>
                  <div className="h-8 w-px bg-gray-200 dark:bg-white/10 mx-2"></div>
                  <div className="flex flex-col">
                    ...
                    <p className="text-3xl font-bold text-gray-500 dark:text-gray-400">
                      {filteredProjects.filter(p => p.energyModel === 'N/A').length}
                    </p>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="mb-6">
          ...
          <div>
            ...
            {(() => {
              const archEligible = filteredProjects.filter(p => p.archVsInt === 'Architecture' && p.isEligible);
              const years = Array.from(new Set(archEligible.map(p => p.reportingYear))).sort((a, b) => b - a);
              const latestYear = years.length > 1 ? years[0] : null;
              
              const currentYearArchEligible = latestYear
                ? archEligible.filter(p => p.reportingYear === latestYear)
                : archEligible;
                
              const totalCount = currentYearArchEligible.length;
              const hasGuidanceLevelCount = currentYearArchEligible.filter(
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                  {filteredProjects.map((p, i) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 text-xs font-bold text-indigo-600 dark:text-indigo-400">{p.reportingYear}</td>
